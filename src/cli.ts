...
import { QAReportGenerator, createQAReportGenerator } from './qa/reporter/index.js';
...
export class VibeFlowCLI {
  ...
  private setupCommands(): void {
    ...
    this.program
      ...
      .action(async (options) => {
        await this.runQAReport(options);
      });
  }
  ...
  private async runQAReport(options: {
    path: string;
    output?: string;
    json?: boolean;
    block?: boolean;
    ...
    try {
      ...
      for (const result of results) {
        generator.addVerification(result);
      }
