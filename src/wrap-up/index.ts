...
`;
  ...
  private async executeSelfImprove(): Promise<SelfImproveResult> {
    ...
    if (this.config!.phases.selfImprove.analyzeErrors) {
      console.log(chalk.gray('  📊 Analyzing error patterns...'));

      try {
        // Get state to analyze errors
        const state = await this.stateMachine.getState();
        const errors = state.errors || [];

        if (errors.length > 0) {
          result.patternsDetected = errors.length;
          console.log(chalk.gray(`    Detected ${errors.length} error patterns`));

          // Generate a simple rule file for error tracking
          const rulesDir = join(homedir(), '.vibe-flow', 'rules');
          await fs.mkdir(rulesDir, { recursive: true });

          const ruleFile = join(rulesDir, `error-patterns-${Date.now()}.json`);
          ...
          console.log(chalk.green(`    Saved error patterns to ${ruleFile}`));
        } else {
          console.log(chalk.gray('    No errors to analyze'));
        }
      } catch (error: any) {
